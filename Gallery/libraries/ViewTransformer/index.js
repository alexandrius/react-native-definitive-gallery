...
export default class ViewTransformer extends React.Component {
   ...
   onResponderMove(evt, gestureState) {
      ...
      let transform = {};
      if (
         gestureState.previousPinch &&
         gestureState.pinch &&
         this.props.enableScale
      ) {
   ...
   performFling(vx, vy) {
      ...
      if (vx > 0) {
         ...
         if (availablePanDistance.right > 0) {
            minX =
               -availablePanDistance.right - this.props.maxOverScrollDistance;
         } else {
            minX = 0;
         }
      }
      if (vy > 0) {
         ...
         if (availablePanDistance.bottom > 0) {
            minY =
               -availablePanDistance.bottom - this.props.maxOverScrollDistance;
         } else {
            minY = 0;
         }
      }
   ...
   animate(targetRect, durationInMillis) {
      ...
      this.state.animator.addListener((state) => {
         ...
         let left =
            fromRect.left + (targetRect.left - fromRect.left) * progress;
         let right =
            fromRect.right + (targetRect.right - fromRect.right) * progress;
         let top = fromRect.top + (targetRect.top - fromRect.top) * progress;
         let bottom =
            fromRect.bottom + (targetRect.bottom - fromRect.bottom) * progress;
